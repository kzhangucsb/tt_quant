...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 25 10:49:52 2020
...
from tensor_layer_quant import TTlinear
        ...
        with tqdm(total=len(train_loader.dataset), desc='Iter {}'.format(epoch)) as bar:
            ...
            for batch_idx, (data, target) in enumerate(train_loader):
                ...
                for n, p in model.named_parameters():
                    if 'factor' in n:
                        if torch.max(torch.abs(p.data)) > 0.875:
                            p.data /= (torch.max(torch.abs(p.data)) / 0.875)
                    if 'bias' in n:
                        p.data.clamp_(-1, 1)
